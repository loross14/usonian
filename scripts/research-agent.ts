...
**Note:** If status is "active", listing_price and listing_source_url are REQUIRED.
...
function generateFlwMonitorReport(): string {
  ...
  const totalValue = flwActive.reduce((sum, p) => sum + (p.listing_price || p.last_sale_price || 0), 0)
...
${FLW_MONITORED_PROPERTIES.map(mp => {
  ...
  const listingPrice = dbProp?.listing_price || 0
  const lastSalePrice = dbProp?.last_sale_price || 0
  const priceMatch = listingPrice === mp.price ? '✓' : '⚠️ PRICE CHANGE'
  const status = dbProp?.status || 'unknown'
  const sourceUrl = dbProp?.listing_source_url || 'No source'

  return `### ${mp.name}
- **Location**: ${mp.location}
- **Recorded Price**: $${mp.price.toLocaleString()}
- **Listing Price**: $${listingPrice.toLocaleString()} ${priceMatch}
- **Last Sale Price**: $${lastSalePrice.toLocaleString()}
- **Source**: ${sourceUrl}
- **Status**: ${status}
...
- **Listing Price**: $
- **Source URL**: ← REQUIRED
- **Last Sale**: $ (year) ← Historical context if known
...
When updating an existing property's price:
1. Update \`listing_price\` to new value
2. Add to \`curator_notes\`: "Price changed from $X to $Y on [date]"
3. Update \`listing_source_url\` to current listing link
